...
spotify_songs |> 
  ...
  geom_vline(aes(xintercept = mean(danceability) - sd(danceability)), color = "blue", linetype = "dashed", size = 1) + geom_vline(aes(xintercept = mean(danceability) + sd(danceability)), color = "blue", linetype = "dashed", size = 1)
...
The distribution of danceability scores in the Spotify dataset is skewed in the left direction. A few very low amounts of danceability push the dataset in the left direction. The mean tells us that an average danceability is about 0.673. The range of data covered by one standard deviation from the mean is about 0.511 to 0.835, which represents one standard deviation above and below the mean.
